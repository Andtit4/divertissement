...
class _QuizzState extends State<Quizz> {
  ...
  getUser() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      emailUser = prefs.getString('prenom').toString();
    });
  ...
  fetchCinemaData() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Erreur de chargement des données de la question ${response.statusCode}');
    }
  }

  fetchResponses(id_question) async {
    String apiLink = link('reponse/search?id_question=$id_question', '');
                    ...
                    builder: (BuildContext context, AsyncSnapshot snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return PageView.builder(
                            ...
                            onPageChanged: (page) {
                              scoreController.incrementPageindex();

                              print(
                                  '--------- ${scoreController.currentPageIndex}  ${data.length}');
                            },
